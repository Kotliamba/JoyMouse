...
class ViewController: NSViewController {
    ...
    var connectedControllers: [GameController] {
        return appDelegate?.controllers.filter { $0.controller != nil } ?? []
    }
    ...
    @objc func controllerDisconnected(_ notification: NSNotification) {
        if let disconnected = notification.object as? GameController,
           self.selectedController === disconnected {
    ...
    @objc func controllerRemoved(_ notification: NSNotification) {
        ...
        let numItems = self.controllerCollectionView.numberOfItems(inSection: 0)
        for i in 0..<numItems {
            if let item = self.controllerCollectionView.item(at: i) as? ControllerViewItem,
               item.controller === gameController {
                self.controllerCollectionView.deselectAll(nil)
                break
            }
        }
    ...
    @objc func controllerIconChanged(_ notification: NSNotification) {
        guard notification.object is GameController else { return }
