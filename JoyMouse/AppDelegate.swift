...
let helperAppID: CFString = "ru.kotliamba.JoyMouseLauncher" as CFString
...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func existingController(for controller: JoyConSwift.Controller) -> GameController? {
        if !controller.serialID.isEmpty {
            return self.controllers.first(where: { $0.data.serialID == controller.serialID })
        }
        
        return self.controllers.first(where: { $0.controller === controller })
    }
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        self.windowController = storyboard.instantiateController(withIdentifier: "JoyMouseWindowController") as? NSWindowController
    ...
    func updateControllersMenu() {
        guard Thread.isMainThread else {
            DispatchQueue.main.async { self.updateControllersMenu() }
            return
        }
    ...
    func connectController(_ controller: JoyConSwift.Controller) {
        if let gameController = self.existingController(for: controller) {
    ...
    func disconnectController(_ controller: JoyConSwift.Controller) {
        if let gameController = self.existingController(for: controller) {
            if gameController.data.serialID?.isEmpty ?? true {
                self.controllers.removeAll(where: { $0 === gameController })
                self.dataManager?.delete(gameController.data)
                NotificationCenter.default.post(name: .controllerRemoved, object: gameController)
                self.updateControllersMenu()
                return
            }
    ...
    func addController(_ controller: JoyConSwift.Controller) {
        ...
        if controllerData.managedObjectContext != nil {
            _ = dataManager.save()
        }
    ...
    func removeController(_ controller: JoyConSwift.Controller) {
        guard let gameController = self.existingController(for: controller) else { return }
