...
class GameController {
    ...
    func setControllerHandler() {
        ...
        if let bodyColor = NSColor(cgColor: controller.bodyColor) {
            self.data.bodyColor = try? NSKeyedArchiver.archivedData(withRootObject: bodyColor, requiringSecureCoding: false)
            self.bodyColor = bodyColor
        }

        if let buttonColor = NSColor(cgColor: controller.buttonColor) {
    ...
    func stickMouseHandler(pos: CGPoint, speed: CGFloat) {
        ...
        let totalBounds = NSScreen.screens.reduce(CGRect.null) { $0.union($1.frame) }
        let newX = min(max(totalBounds.minX, mousePos.x + pos.x * speed), totalBounds.maxX)
        let newY = min(max(primaryHeight - totalBounds.maxY, primaryHeight - mousePos.y - pos.y * speed), primaryHeight - totalBounds.minY)
